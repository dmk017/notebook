...
from objects.templates_objects import (
    add_object_template,
    decline_objects_template,
    decline_objects_filters_template,
)
from objects.sends_objects import send_add_object_request, send_decline_objects_request
...
def test_decline_object(authenticated_client, setup_data):
    ...
    decline_request = decline_objects_template(
        [response_add_object1_json["_id"], response_add_object2_json["_id"]]
    )
    response_decline = send_decline_objects_request(
        authenticated_client, decline_request
    )

    assert response_decline.status_code == 200
    assert response_decline.text == "2"


def test_decline_object_with_filters(authenticated_client, setup_data):
    unique_suffix, model_id = setup_data
    properties_for_object_1 = [
        {
            "property_name": f"ФИО_{unique_suffix}",
    ...
    decline_objects_request = decline_objects_filters_template(filters)
    response_decline_objects = send_decline_objects_request(
        authenticated_client, decline_objects_request, []
    )

    assert response_decline_objects.status_code == 200
    assert response_decline_objects.text == "3"


def test_decline_object_with_invalid_filter(authenticated_client, setup_data):
    unique_suffix, model_id = setup_data
    ...
    approve_objects_request = decline_objects_filters_template(filters)

    with pytest.raises(
        TypeError, match="'NoneType' object is not subscriptable"
    ) as exc_info:
        send_decline_objects_request(authenticated_client, approve_objects_request, [])

    assert "object is not subscriptable" in str(exc_info.value)
