...
from objects.sends_objects import send_add_object_request, send_approve_objects_request
...
def test_approve_object(authenticated_client, setup_data):
    ...
    approve_request = approve_objects_template(
        [response_add_object1_json["_id"], response_add_object2_json["_id"]]
    )
    response_approve = send_approve_objects_request(
        authenticated_client, approve_request
    )

    assert response_approve.status_code == 200
    assert response_approve.text == "2"


def test_approve_object_with_filters(authenticated_client, setup_data):
    unique_suffix, model_id = setup_data
    properties_for_object_1 = [
        {
            "property_name": f"ФИО_{unique_suffix}",
    ...
    assert response_approve_objects.text == "7"


def test_approve_object_with_invalid_filter(authenticated_client, setup_data):
    unique_suffix, model_id = setup_data
    ...
    with pytest.raises(
        ...
        send_approve_objects_request(authenticated_client, approve_objects_request, [])
